#define _POSIX_C_SOURCE 200112L  // 指定POSIX.1-2001标准
...
int main(int argc, char *argv[]) {
    ...
    // 条件编译：优先使用POSIX标准版本，否则使用Linux版本
#ifdef POSIX_FADV_SEQUENTIAL
    if (posix_fadvise(fd, offset, len, POSIX_FADV_SEQUENTIAL) < 0) {
        perror("fadvise");
    }
#else
    if (posix_fadvise(fd, offset, len, FADV_SEQUENTIAL) < 0) {
        perror("fadvise");
    }
#endif
